...
function g!(G, theta, X, y)
    ...
    sigmoids = 1.0 ./ (1.0 .+ exp.(-(theta[1] .+ theta[2:end]' * X)))'
    G[1] = -sum(y .- sigmoids)
    G[2:end] = ((sigmoids .- y)' * X')' + 0.5 * theta[2:end]
end
...
point = zeros(dim + 1)
result = optimize(t -> f(t, trainData, trainLabels), (G, t) -> g!(G, t, trainData, trainLabels), point,
LBFGS(linesearch=LineSearches.BackTracking()), Optim.Options(x_tol = 0, f_tol = 0, g_tol = 0, iterations = 10))
print(result)
print("\n")

point = zeros(dim + 1)
result = @time optimize(t -> f(t, trainData, trainLabels), (G, t) -> g!(G, t, trainData, trainLabels), point,
    LBFGS(linesearch=LineSearches.BackTracking()), Optim.Options(x_tol = 0, f_tol = 0, g_tol = 0, iterations = 10))
